...
export default function FormPlayers(props: Props) {
	...
	const handleChallenge = (e: any) => {
		...
		if (players.player1 === players.player2) {
			toast.error('No te puedes jugar contigo mismo')
		}
		if (!isValidBlockchainAddress(players.player2)) {
			toast.error('La dirección no tiene el formato correcto')
		}
		startGame(players.player1 as string, players.player2)
	}
				...
				<Card className='w-96 h-[auto] rounded-xl bg-[#0F0D0C]/50 backdrop-blur border-rose-400 border-2'>
					<CardContent className='space-y-4 flex justify-center flex-col items-center'>
						...
						>
							...
							<input
								type='text'
								className='w-full rounded-xl p-3 bg-[#37005B] text-white'
								placeholder='0x00..'
								onChange={e =>
							...
							/>
							<button
								className='bg-[#37005B] w-full rounded-xl text-white p-2 items-center space-x-2 flex'
								ref={glitchButton}
							>
